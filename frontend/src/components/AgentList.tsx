...
const AgentList: React.FC = () => {
    ...
    const { isOpen: isAddAgentModalOpen, onOpen: onAddAgentModalOpen, onClose: onAddClose } = useDisclosure();
    const { isOpen: isEditOpen, onOpen: onEditOpen, onClose: onEditClose } = useDisclosure();
    const [selectedAgent, setSelectedAgent] = useState<Agent | null>(null);
    const [cliPromptModalOpen, setCliPromptModalOpen] = useState(false);
    const [currentCliPrompt, setCurrentCliPrompt] = useState({ text: '', agentName: '' });
    ...
    const getAgentStats = (agentId: string) => {
        ...
        const isActive = agentTasks.some((task) => task.status !== TaskStatus.COMPLETED);
        const status = isActive ? "Active" : "Idle";
        const statusColorScheme = status === 'Active' ? 'green' : 'gray';
        const statusClassName = status === 'Active' ? styles.agentStatusActive : styles.agentStatusIdle;
        return {
            taskCount,
            projectCount,
            projectNames,
    ...
    if (agentsLoading) return <Text className={styles.loadingText}>Loading agents...</Text>;
    if (agentsError) return <Text className={styles.errorText}>Error loading agents: {agentsError}</Text>;

    return (
        <VStack className={styles.agentListContainer}>
            <AgentListHeader agents={agents} tasks={tasks} onAddAgentClick={onAddAgentModalOpen} />

            {filteredAgents.length === 0 && (
                <Box className={styles.noAgentsTextContainer}>
                     <Text className={styles.noAgentsText}>No agents found matching your criteria.</Text>
                </Box>
            )}

            {filteredAgents.map((agent) => {
                const agentStats: AgentStats = getAgentStats(agent.id);
