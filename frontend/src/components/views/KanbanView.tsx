...
const KanbanView: React.FC<KanbanViewProps> = ({ 
    ...
    const columnHeaderBg = useColorModeValue('gray.200', 'gray.900');
    ...
    return (
        ...
        >
            <Box w="100%" overflowX="auto" py={4} px={{ base: 2, md: 4 }} ref={boardRef} bg="kanban.column.bg" position="relative">
                ...
                <Flex direction="row" gap={{ base: 4, md: 6 }} w="max-content" minW="100%" px={1}>
...
interface KanbanColumnContentProps {
    ...
    columnHeaderBg: string; // Added
    columnBg: string; // Added
}
...
const KanbanColumnContent: React.FC<KanbanColumnContentProps> = ({ 
    ...
    cardBg, 
    cardBorderPrimary,
    columnHeaderBg, // Added
    columnBg // Added
}) => {
    return (
        <VStack
            key={column.id} // Key should be on the mapped element if this is not used in a map directly
            alignItems="stretch"
            p={compactView ? 1 : 2}
            bg={columnBg} // Use themed color
            borderRadius="lg"
            boxShadow="md"
            ...
            height="100%" // Ensure VStack fills the Flex container height
            data-status-key={column.id} // For e2e tests or specific styling
        ...
        >
            <Flex 
                ...
                px={compactView ? 1 : 2}
                pt={compactView ? 1 : 0}
                bg={columnHeaderBg} // Use themed color
                borderTopRadius="lg" 
                mx={compactView ? -1 : -2} 
                mt={compactView ? -1 : -2}
            >
                <Heading size={compactView ? "xs" : "sm"} color={"text.primary"} textTransform="uppercase" letterSpacing="wide">
                    {column.title}
                </Heading>
                <Text fontSize={compactView ? "xs" : "sm"} color={"text.secondary"} fontWeight="bold">
                    {column.tasks.length}
                </Text>
            </Flex>
                ...
                <VStack 
                    ...
                    css={{
                        ...
                        '&::-webkit-scrollbar-track': {
                            background: defaultThemeBg, // Use passed prop
                        },
                        '&::-webkit-scrollbar-thumb': {
                            background: cardBorderPrimary, // Use passed prop
                            borderRadius: '8px',
                        },
                        '&::-webkit-scrollbar-thumb:hover': {
                            background: cardBg, // Use passed prop
                        },
                    }}
                >
                    {column.tasks.length === 0 && (
                        <Text fontSize="sm" color="text.muted" textAlign="center" pt={4}>
                            No tasks here.
                        </Text>
                    )}
