 ...
 * />
 */
const TaskItemDetailsSection: React.FC<
  Omit<TaskItemDetailsSectionProps, 'statusInfo'>
> = ({
  ...
  const [fileSortDirection, setFileSortDirection] = useState<
    string | undefined
  >(undefined);
  const [fileFilterFilename, setFileFilterFilename] = useState<
    string | undefined
  >(undefined);
  ...
  const {
    ...
    onOpen: onDependencyModalOpen,
    onClose: onDependencyModalClose,
  } = useDisclosure();
  ...
  const [addDependencyError, setAddDependencyError] = useState<string | null>(
    null
  );
  ...
  const refreshComments = async () => {
    if (!task.project_id || task.task_number === undefined) return;
  ...
  const fetchTaskDetails = async () => {
    ...
    try {
      ...
      const preds = await getTaskPredecessors(
        task.project_id,
        task.task_number
      );
  ...
  const handleAssociateFile = async () => {
    ...
    try {
      await associateFileWithTask(task.project_id, task.task_number, {
        file_id: fileIdInput,
      });
      fetchTaskDetails();
      setFileIdInput('');
      ...
      setAddFileError(
        err.message || 'An error occurred while associating file.'
      );
    } finally {
  ...
  const handleDisassociateFile = async (fileId: string) => {
    if (
      !window.confirm(
        `Are you sure you want to disassociate file ${fileId} from task ${task.task_number}?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setDetailsError(
        err.message || 'An error occurred while disassociating file.'
      );
      setIsLoadingDetails(false);
    }
  };
  ...
  const handleAddDependency = async () => {
    ...
    try {
      ...
      setAddDependencyError(
        err.message || 'An error occurred while adding dependency.'
      );
    } finally {
  ...
  const handleRemoveDependency = async (dependency: TaskDependency) => {
    ...
    try {
      ...
      setDetailsError(
        err.message || 'An error occurred while removing dependency.'
      );
      setIsLoadingDetails(false);
    }
  };
  ...
  const handleAddComment = async (content: string) => {
    ...
    try {
      await addTaskComment(task.project_id, task.task_number, { content });
      await refreshComments();
    } catch (err: any) {
      console.error('Failed to add comment:', err);
      setAddCommentError(
        err.message || 'An error occurred while adding the comment.'
      );
    } finally {
    ...
    <VStack spacing={compact ? 1 : 2} align="stretch">
      ...
      >
        ...
        <TaskStatusTag
          ...
          fontWeight={
            (styles.statusTagStyle as { fontWeight?: string | number })
              ?.fontWeight
          }
        ...
        {(task.agent_name || task.agent_id) && (
          <TaskAgentTag
            ...
            fontWeight={
              (styles.agentTagStyle as { fontWeight: string | number })
                .fontWeight
            }
          />
        )}
      ...
      {!isLoadingDetails && !detailsError && fileAssociations.length > 0 && (
        <Box>
          <HStack justifyContent="space-between">
            ...
            <Button size="xs" onClick={onFileModalOpen}>
              Add File
            </Button>
          </HStack>
        ...
        (predecessors.length > 0 || successors.length > 0) && (
          <Box>
            <HStack justifyContent="space-between">
              ...
              <Button size="xs" onClick={onDependencyModalOpen}>
                Add Dependency
              </Button>
            </HStack>
            ...
            {predecessors.length === 0 && successors.length === 0 && (
              <Text fontSize="sm" mt={1}>
                No dependencies or blocking tasks.
              </Text>
            )}
          </Box>
        )}
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button
              ...
              isDisabled={isAddingFile}
            >
              Cancel
            </Button>
            <Button
              colorScheme="blue"
              onClick={handleAssociateFile}
      ...
      <Modal isOpen={isDependencyModalOpen} onClose={onDependencyModalClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={4}>
              <Text>
                This task ({task.project_id}/{task.task_number}) will depend on
                another task.
              </Text>
              ...
              <FormControl id="dependency-type-input">
                ...
                >
                  <option value="finishes_before_starts">
                    Finishes Before Starts
                  </option>
                </Select>
              </FormControl>
              {addDependencyError && (
                <Text color="red.500">{addDependencyError}</Text>
              )}
            </VStack>
          </ModalBody>
          ...
          <ModalFooter>
            <Button
              variant="ghost"
              mr={3}
              onClick={onDependencyModalClose}
              isDisabled={isAddingDependency}
            >
              Cancel
            </Button>
            <Button
