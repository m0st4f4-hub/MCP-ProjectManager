...
const TaskList: React.FC = () => {
  ...
  const applyAllFilters = useCallback(
    (task: Task, currentFilters: TaskFilters): boolean => {
      // Search filter
      ...
      if (
        currentFilters.hideCompleted &&
        getTaskCategory(task) === "completed"
      ) {
        // Assumes getTaskCategory is available
        return false;
      }
  ...
  const groupedAndFilteredTasks: GroupedTasks = useMemo(() => {
    ...
    const getStatusSubgroupsForProjectOrAgent = (currentTasks: Task[]) => [
      {
        ...
        name: `Active Tasks (${
          currentTasks.filter((t) => {
            const attrs = statusUtils.getStatusAttributes(
              t.status as statusUtils.StatusID,
            );
            return !(
              attrs?.category === "completed" ||
              t.status?.startsWith("COMPLETED_")
            );
          }).length
        })`,
        tasks: sortTasks(
    ...
    if (effectiveGroupBy === "status") {
      ...
      topLevelTasks.forEach((task) => {
        // Use topLevelTasks which respects top_level_only for list view
        const currentTaskStatusId = (task.status ||
          "TO_DO") as statusUtils.StatusID; // Ensure it's a valid StatusID
      ...
      const allStatusIdsInTasks = Object.keys(
        tasksByStatusId,
      ) as statusUtils.StatusID[];
      ...
      allStatusIdsInTasks.sort((a, b) => {
        ...
        const aDisplayName =
          statusUtils.getDisplayableStatus(a)?.displayName || a;
        const bDisplayName =
          statusUtils.getDisplayableStatus(b)?.displayName || b;
      ...
      const statusGroups = allStatusIdsInTasks
        .map((statusIdKey) => {
          ...
          const { displayName } = statusUtils.getDisplayableStatus(
            statusIdKey,
          ) || { displayName: statusIdKey };
          return {
            id: statusIdKey,
    ...
    if (effectiveGroupBy === "project") {
      ...
      const projectGroups = projects.map((project) => ({
        ...
        subgroups: getStatusSubgroupsForProjectOrAgent(
    ...
    effectiveGroupBy,
    sortOptions,
  ]); // Removed filters.top_level_only as it's handled in filteredTasksForListView
  ...
  if (error) {
    return (
      <TaskError error={typeof error === "string" ? error : String(error)} />
    );
  }
