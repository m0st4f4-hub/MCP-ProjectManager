...
export const createTask = async (taskData: TaskCreateData): Promise<Task> => {
  const rawTask = await request<RawTask>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/tasks/`, { method: 'POST', body: JSON.stringify(taskData) });
  const statusId = normalizeToStatusID(rawTask.status, !!rawTask.completed);
  return {
    ...rawTask,
    id: String(rawTask.id),
...
export const updateTask = async (task_id: string, taskData: TaskUpdateData): Promise<Task> => {
  ...
  const rawTask = await request<RawTask>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/tasks/${task_id}`, { method: 'PUT', body: JSON.stringify(payload) });
  const statusId = normalizeToStatusID(rawTask.status, !!rawTask.completed);
  return {
    ...rawTask,
    id: String(rawTask.id),
...
export const deleteTask = async (task_id: string): Promise<Task> => {
  const rawTask = await request<RawTask>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/tasks/${task_id}`, { method: 'DELETE' });
  if (!rawTask) {
...
export const archiveTask = async (taskId: string): Promise<Task> => {
    const rawTask = await request<RawTask>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/tasks/${taskId}/archive`, { method: 'POST' });
    const statusId = normalizeToStatusID(rawTask.status, !!rawTask.completed);
    return {
        ...rawTask,
        id: String(rawTask.id),
...
export const unarchiveTask = async (taskId: string): Promise<Task> => {
    const rawTask = await request<RawTask>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/tasks/${taskId}/unarchive`, { method: 'POST' });
    const statusId = normalizeToStatusID(rawTask.status, !!rawTask.completed);
    return {
        ...rawTask,
        id: String(rawTask.id),
