...
export const getAgentById = async (agent_id: string): Promise<Agent> => {
  const rawAgent = await request<RawAgent>(
    `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8080"}/agents/${agent_id}`,
  );
  return {
    ...rawAgent,
    id: String(rawAgent.id),
...
export const getAgentByName = async (agent_name: string): Promise<Agent> => {
  const rawAgent = await request<RawAgent>(
    `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8080"}/agents/name/${agent_name}`,
  );
  return {
    ...rawAgent,
    id: String(rawAgent.id),
...
export const createAgent = async (name: string): Promise<Agent> => {
  const rawAgent = await request<RawAgent>(
    `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8080"}/agents/`,
