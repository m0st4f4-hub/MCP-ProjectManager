...
export const normalizeToStatusID = (backendStatus: string | null | undefined, completedFlag: boolean): StatusID => {
    if (completedFlag) {
        return 'COMPLETED';
    }
    if (backendStatus) {
        ...
        console.warn(`Unknown backend status string: "${backendStatus}". Defaulting to "TO_DO".`);
        return 'TO_DO';
    }
