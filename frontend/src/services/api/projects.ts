import { Project, ProjectCreateData, ProjectUpdateData, ProjectFilters } from "@/types";
...
export const createProject = async (projectData: ProjectCreateData): Promise<Project> => {
    const rawProject = await request<RawProject>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/projects/`, { method: 'POST', body: JSON.stringify(projectData) });
    return {
        ...rawProject,
        id: String(rawProject.id),
        name: String(rawProject.name || ''),
...
export const updateProject = async (project_id: string, projectData: ProjectUpdateData): Promise<Project> => {
    const rawProject = await request<RawProject>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/projects/${project_id}`, { method: 'PUT', body: JSON.stringify(projectData) });
    return {
        ...rawProject,
        id: String(rawProject.id),
        name: String(rawProject.name || ''),
...
export const deleteProject = async (project_id: string): Promise<Project> => {
    const rawProject = await request<RawProject>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/projects/${project_id}`, { method: 'DELETE' });
    return {
        ...rawProject,
        id: String(rawProject.id),
        name: String(rawProject.name || ''),
...
export const archiveProject = async (projectId: string): Promise<Project> => {
    const rawProject = await request<RawProject>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/projects/${projectId}/archive`, { method: 'POST' });
    return {
        ...rawProject,
        id: String(rawProject.id),
        name: String(rawProject.name || ''),
...
export const unarchiveProject = async (projectId: string): Promise<Project> => {
    const rawProject = await request<RawProject>(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080'}/projects/${projectId}/unarchive`, { method: 'POST' });
    return {
        ...rawProject,
        id: String(rawProject.id),
        name: String(rawProject.name || ''),
