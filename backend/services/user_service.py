...
from ..schemas.user import UserCreate, UserUpdate # Added direct imports
...
from .exceptions import EntityNotFoundError, DuplicateEntityError, ValidationError, AuthorizationError
...
class UserService:
 ...
 async def get_user_by_username(self, username: str) -> Optional[models.User]: # Make async
 # Delegate to CRUD
 ...
 if await username_exists(self.db, user_create.username): # Await username_exists
 # Use the proper service exception
 raise DuplicateEntityError("User", user_create.username)

 # Use transaction context manager
 # Need to check if service_transaction works with AsyncSession or needs an async version
 # For now, assuming it might need adjustment, will read utils next.
 # Keeping the structure for now, but it might change.
 # with service_transaction(self.db, "create_user") as tx_db: # This might need adjustment for async
 try:
 # Delegate to CRUD create function if username is unique
 # Since CRUD functions now handle their own commits/rollbacks, no need for service_transaction here
 ...
 async def update_user(self, user_id: str, user_update: UserUpdate) -> Optional[models.User]: # Make async
 # Delegate to CRUD update function
 return await update_user(self.db, user_id, user_update) # Await CRUD call

 async def delete_user(self, user_id: str) -> Optional[models.User]: # Make async
