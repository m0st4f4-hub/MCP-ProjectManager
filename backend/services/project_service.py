...
from ..schemas.project import ProjectCreate, ProjectUpdate, ProjectMemberCreate, ProjectFileAssociationCreate, Project
# Import schema for ProjectTemplate
from ..schemas.project_template import ProjectTemplate
...
class ProjectService:
 ...
 # Use transaction context manager (assuming it handles async)
 # Note: service_transaction context manager likely needs to be async or removed if not truly handling
 # async sessions correctly. Will proceed assuming it can be used with async for now.
 # If issues persist, this context manager is a potential area to investigate.
 # Original: async with service_transaction(self.db, "create_project") as tx_db:
 # Diagnostic: Explicitly await the context manager
 async with service_transaction(self.db, "create_project") as tx_db:
 # Create the basic project first (await the async CRUD call)
 db_project = await create_project(tx_db, project)
 ...
 **member_data, project_id=db_project.id
 ...
 # Await the async CRUD call
 await project_members_crud.add_project_member(tx_db, member_create_schema)
 ...
 # Use selectinload to eagerly load necessary relationships including nested ones
 # We need project_members and the user associated with each project member
 # Query using the transaction session (tx_db)
 ...
 if db_project_loaded is None:
 # This should not happen if create_project succeeded, but handle defensively
 raise EntityNotFoundError("Project", db_project.id)

 # Manually extract data into a dictionary to ensure no session access after return
 project_data = {
 ...
 "task_count": db_project_loaded.task_count, # Should be loaded integer
 # Manually extract project_members and their users
 "project_members": [
 {
 ...
 # Do NOT include User relationships here unless explicitly needed and loaded
 } if member.user else None,
 # Do NOT include Project relationship here (circular)
 } for member in db_project_loaded.project_members
 ],
 # Project schema does NOT include tasks list, only task_count.
 # So, no need to extract tasks list here.
 }

 # Create the Pydantic model from the dictionary
 project_pydantic = Project(**project_data)

 # Return the Pydantic model after the transaction ends
 return project_pydantic
 ...
 # Check for name conflict if name is being changed (await the async method)
 ...
 # but keep for safety
 raise EntityNotFoundError("Project", project_id)
 # Refresh the updated project (await the refresh)
 ...
 # Convert to async method and use await
 async def get_project_members(self, project_id: str) -> List[models.ProjectMember]:
 # Delegate to CRUD and await
 ...
 # Delegate to CRUD in project_file_associations and await
 ...
 # Convert to async method and use await
 async def get_project_files(self, project_id: str) -> List[models.ProjectFileAssociation]:
 # Delegate to CRUD in project_file_associations and await
 return await get_project_files(self.db, project_id)

 # Convert to async method and use await
 async def get_project_file_association(self, project_id: str, file_memory_entity_id: int) -> Optional[models.ProjectFileAssociation]:
 # Delegate to CRUD in project_file_associations and await
 return await get_project_file_association(self.db, project_id, file_memory_entity_id)

 # Convert to async method and use await
 async def get_tasks_by_project(self, project_id: str, search: Optional[str] = None, status: Optional[str] = None, is_archived: Optional[bool] = False) -> List[models.Task]:
 # Delegate to CRUD and await
