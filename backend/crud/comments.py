...
import uuid
import json # Import json for details field handling
...
from sqlalchemy.orm import joinedload as async_joinedload # Use async_joinedload for clarity
...
async def get_comments_by_task(
    ...
    """Retrieve multiple comments for a specific task with pagination."""
    # Eagerly load author (User)
    # Use async execute with select, options, filter, order_by, offset, and limit
...
async def create_comment(db: AsyncSession, comment_create: CommentCreate) -> models.Comment:
    ...
    if comment_create.task_project_id and comment_create.task_task_number is not None:
        # Await the async validation function calls
    ...
    # Await the async validation function call
    ...
    await db.commit()
    await db.refresh(db_comment_entry)
    # Refresh author relationship (Await the async refresh)
