# Task ID: 211  # Agent Role: Agent 1  # Request ID: (Inherited from Overmind)  # Project: task-manager  # Timestamp: 2025-05-24T12:00:00Z
...
async def get_all_task_dependencies(db: AsyncSession, project_id: str, skip: int = 0, limit: int = 100) -> List[TaskDependency]:
    """Get all task dependencies for a project."""
    result = await db.execute(select(TaskDependency).filter(
        and_(
            TaskDependency.predecessor_project_id == project_id,
            TaskDependency.successor_project_id == project_id
        )
    ).offset(skip).limit(limit))
    return result.scalars().all()
...
async def create_task_dependency(db: AsyncSession, task_dependency: TaskDependencyCreate) -> TaskDependency:
    """Create a new task dependency, preventing self or circular dependencies."""  # Check for self-dependency
    ...
    task_dependency.successor_project_id, task_dependency.successor_task_number):
        raise HTTPException(status_code=400, detail="A task cannot be dependent on itself")  # Check for circular dependency
