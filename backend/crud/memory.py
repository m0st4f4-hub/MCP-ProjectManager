...
from sqlalchemy import text, select, update, delete, and_ # Import and_ for relationships
from sqlalchemy.ext.asyncio import AsyncSession # Import AsyncSession
from backend.models.memory import MemoryRelation as MemoryRelationshipModel # Import MemoryRelation model
...
async def get_entity_by_name(db: AsyncSession, name: str) -> Optional[MemoryEntityModel]:
    ...
    logger.debug(f"[DEBUG] get_entity_by_name called with name: {name}") # Debug print
    # Use json_extract for SQLite compatibility
    result = await db.execute(text("SELECT memory_entities.* FROM memory_entities WHERE json_extract(memory_entities.entity_metadata, '$.name') = :name").params(name=name))
    entity = result.scalar_one_or_none()
    logger.debug(f"[DEBUG] get_entity_by_name returned: {entity}") # Debug print
    return entity
...
async def update_memory_entity(db: AsyncSession, entity_id: int, entity_update: MemoryEntityUpdate) -> Optional[MemoryEntityModel]:
    ...
    logger.debug(f"[DEBUG] update_memory_entity called with entity_id: {entity_id}, update_data: {entity_update.model_dump_json()}") # Debug print
...
async def delete_memory_entity(db: AsyncSession, entity_id: int) -> bool:
    ...
    logger.debug(f"[DEBUG] delete_memory_entity called with entity_id: {entity_id}") # Debug print
    # Fetch the entity asynchronously
    db_entity = await get_memory_entity(db, entity_id) 
    if db_entity:
...
async def get_memory_relations_between_entities(db: AsyncSession, from_entity_id: int, to_entity_id: int, relation_type: str):
    """Retrieve relationships between two entities."""
    # This function was implicitly used, converting to async and adding debug
    logger.debug(f"[DEBUG] get_memory_relations_between_entities called with from: {from_entity_id}, to: {to_entity_id}, type: {relation_type}") # Debug print
    # from backend.models.memory import MemoryRelationship as MemoryRelationshipModel # Import inside function to avoid circular dep if needed - REMOVED
...
async def create_memory_relation(db: AsyncSession, relation: MemoryRelationCreate):
    ...
    # This function was implicitly used, converting to async and adding debug
    logger.debug(f"[DEBUG] create_memory_relation called with relation: {relation.model_dump_json()}") # Debug print
    # from backend.models.memory import MemoryRelationship as MemoryRelationshipModel # Import inside function - REMOVED
...
async def delete_memory_relation(db: AsyncSession, relation_id: int):
     """Delete a memory relationship by ID."""
     # This function was implicitly used, converting to async and adding debug
     logger.debug(f"[DEBUG] delete_memory_relation called with relation_id: {relation_id}") # Debug print
     # from backend.models.memory import MemoryRelationship as MemoryRelationshipModel # Import inside function - REMOVED
     stmt = delete(MemoryRelationshipModel).where(MemoryRelationshipModel.id == relation_id)
     result = await db.execute(stmt)
     # No commit needed here if caller handles it, but adding for completeness
     await db.commit()
