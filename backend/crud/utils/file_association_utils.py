...
def file_entity_exists(db: Session, file_memory_entity_id: int) -> bool:
    ...
    """
    return crud.memory.get_memory_entity_by_id(db, entity_id=file_memory_entity_id) is not None
...
def project_entity_exists(db: Session, project_id: str) -> bool:
    ...
    return crud.memory.get_memory_entity_by_name(db, name=project_entity_name) is not None
