...
def upgrade() -> None:
    ...
    sa.Column('id', sa.String(length=32), nullable=False),
    sa.Column('name', sa.String(), nullable=False),
    sa.Column('description', sa.Text(), nullable=True),  # Add other columns as needed
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index(op.f('ix_task_statuses_order'), 'task_statuses', ['order'], unique=True)  # Create TaskStatusEnum for SQLite
    task_status_enum = sa.Enum('TO_DO', 'IN_PROGRESS', 'COMPLETED', 'BLOCKED', 'CANCELLED',
    name='taskstatusenum')
...
def downgrade() -> None:
    """Downgrade schema."""  # Drop tables in reverse order of creation (respecting foreign key constraints)
