...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_tasks_title'), 'tasks', [
                    'title'], unique=False)  # Assuming title is indexed
    op.create_index(op.f('ix_tasks_description'), 'tasks', [
                    'description'], unique=False)  # Assuming description is indexed
    op.create_index(op.f('ix_tasks_project_id'), 'tasks',
                    ['project_id'], unique=False)
    op.create_index(op.f('ix_tasks_agent_id'), 'tasks',
                    ['agent_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.create_table('tasks',
                    ...
                    # Restoring agent_name
                    sa.Column('agent_name', sa.String(), nullable=True),
                    sa.Column('parent_task_id', sa.Integer(),
                              nullable=True),  # Integer parent task ID
                    sa.ForeignKeyConstraint(
                        ['agent_name'], ['agents.name'], ),  # FK to agents.name
                    sa.ForeignKeyConstraint(
                        ['parent_task_id'], ['tasks.id'], ),
                    sa.ForeignKeyConstraint(['project_id'], ['projects.id'], ),
                    sa.PrimaryKeyConstraint('id')
    ...
    op.create_index(op.f('ix_tasks_agent_name'), 'tasks', [
                    'agent_name'], unique=False)  # Index for agent_name
    op.create_index(op.f('ix_tasks_parent_task_id'), 'tasks',
                    ['parent_task_id'], unique=False)
    # ### end Alembic commands ###
