...
class RoleChecker:
    ...
    def __init__(self, allowed_roles: List[UserRoleEnum]):
        ...
        print(f"[AUTH DEBUG] Initialized RoleChecker with allowed roles: {[role.value for role in allowed_roles]}")
    ...
    async def __call__(self, current_user: UserModel = Depends(get_current_active_user)):
        ...
        print(f"[AUTH DEBUG] User {current_user.username} has roles: {user_role_values}")
        ...
        if not any(allowed_role.value in user_role_values for allowed_role in self.allowed_roles):
            print(f"[AUTH DEBUG] User {current_user.username} does not have required roles. Required: {[role.value for role in self.allowed_roles]}")
            raise HTTPException(
