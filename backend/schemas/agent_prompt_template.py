...
class AgentPromptTemplateBase(BaseModel):
    ...
    template_content: str = Field(
        ..., description="Content of the prompt template."
    )
    context_requirements: Optional[str] = Field(
        None, description="Context requirements for the template."
    )
    is_active: bool = Field(
        True, description="Whether the template is active."
    )


class AgentPromptTemplateCreate(AgentPromptTemplateBase):
    """Schema for creating a new agent prompt template."""
    agent_role_id: str = Field(
        ..., description="ID of the associated agent role."
    )
...
class AgentPromptTemplateUpdate(BaseModel):
    ...
    template_name: Optional[str] = Field(
        None, description="Updated template name."
    )
    template_content: Optional[str] = Field(
        None, description="Updated template content."
    )
    context_requirements: Optional[str] = Field(
        None, description="Updated context requirements."
    )
