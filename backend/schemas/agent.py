...
# from typing import Union, Any, Dict # Unused imports removed
from datetime import datetime

# Forward references for relationships (avoid circular imports)
# Task will be resolved by Pydantic when all modules are loaded
...
class Agent(AgentBase):
    ...
    # Temporarily comment out tasks relationship to fix circular import
...
class AgentRuleBase(BaseModel):
    ...
    is_active: bool = Field(
        True, description="Whether the rule is currently active."
    )
...
class AgentRuleUpdate(BaseModel):
    ...
    agent_id: Optional[str] = Field(
        None, description="New agent ID for the rule."
    )
    rule_type: Optional[str] = Field(
        None, description="New type for the rule."
    )
    rule_content: Optional[str] = Field(
        None, description="New content for the rule."
    )
    is_active: Optional[bool] = Field(
        None, description="New active status for the rule."
    )
...
class AgentRule(AgentRuleBase):
    ...
    id: str = Field(
        ..., description="Unique identifier for the agent rule."
    )
