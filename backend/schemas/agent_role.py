...
from datetime import datetime  # --- Agent Role Schemas ---


class AgentRoleBase(BaseModel):
    """Base schema for agent role attributes."""
    name: str = Field(
        ..., description="The unique name of the agent role."
    )
    display_name: str = Field(
        ..., description="Display name for the agent role."
    )
    primary_purpose: str = Field(
        ..., description="Primary purpose of the agent role."
    )
    is_active: bool = Field(
        True, description="Whether the role is active."
    )
...
class AgentRoleUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, description="Updated name of the role."
    )
    display_name: Optional[str] = Field(
        None, description="Updated display name."
    )
    primary_purpose: Optional[str] = Field(
        None, description="Updated primary purpose."
    )
