...
from backend.crud.comments import (
    ...
    create_comment,
    update_comment,
    delete_comment  # Import async CRUD functions  # Import standardized API response models
)
...
async def get_comments_by_task_endpoint(
    ...
    comments = await get_comments_by_task(db, task_project_id=task_project_id, task_task_number=task_task_number, skip=skip, limit=limit)  # You might want to implement total count for proper pagination in a real app
    return ListResponse[Comment](
        ...
        message=f"Retrieved {len(comments)} comments for task"
