...
from ..database import Base, get_db # Adjust path if necessary for your project structure
...
def create_test_agent(db: Session, name: str = "Test Agent", role: str = "Test Role", project_id: Optional[str] = None):
    ...
    if project_id is None:
        ...
        default_project = create_test_project(db, name=f"Default Project for {name}")
        project_id = default_project.id
    agent_schema = schemas.AgentCreate(name=name, role=role, project_id=project_id)
...
def apply_db_override(fastapi_app: FastAPI, TestingSessionLocal):
    ...
    def get_db_override_with_session_factory():
        yield from override_get_db_for_app(TestingSessionLocal) # Pass the factory
