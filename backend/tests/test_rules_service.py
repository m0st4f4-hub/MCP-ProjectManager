...
services.rules_service.RulesService.get_agent_prompt = services.rules_service.get_agent_prompt_method
services.rules_service.RulesService.log_rule_violation = services.rules_service.log_rule_violation_method
services.rules_service.RulesService.validate_agent_task = services.rules_service.validate_agent_task_method
services.rules_service.RulesService.check_agent_capabilities = services.rules_service.check_agent_capabilities_method
services.rules_service.RulesService.get_error_protocol = services.rules_service.get_error_protocol_method
services.rules_service.RulesService.get_universal_mandates_for_prompt = services.rules_service.get_universal_mandates_for_prompt_method
def test_get_agent_prompt_calls_crud():
    session = MagicMock()
    service = RulesService(session)
    with patch("backend.services.rules_service.crud_rules") as mock_crud:
        mock_crud.generate_agent_prompt_from_rules.return_value = "prompt"
        result = service.get_agent_prompt("Agent", {"k": 1}, [])
        mock_crud.generate_agent_prompt_from_rules.assert_called_once_with(session, "Agent", {"k": 1}, [])
...
def test_validate_agent_task_logs_violations():
    ...
    with patch("backend.services.rules_service.crud_rules") as mock_crud:
        mock_crud.validate_task_against_agent_rules.return_value = violations
        with patch.object(service, "log_rule_violation", return_value=SimpleNamespace(__dict__={"v": "v"})) as mock_log:
            result = service.validate_agent_task("Agent", {"task_project_id": "p", "task_number": 1})
            mock_crud.validate_task_against_agent_rules.assert_called_once_with(session, "Agent", {"task_project_id": "p", "task_number": 1})
...
def test_check_agent_capabilities():
    ...
    # Patch the CRUD module directly in the services module
    with patch("backend.services.rules_service.crud_rules") as mock_crud:
        mock_crud.get_agent_role_with_details.return_value = role
        service = RulesService(session)
...
def test_get_universal_mandates_for_prompt():
    ...
    with patch("backend.services.rules_service.crud_rules") as mock_crud:
        mock_crud.get_universal_mandates.return_value = mandates
        service = RulesService(session)
