...
async def test_create_and_get_project(async_db_session: AsyncSession):
 ...
 logger.debug(f"[DEBUG test_create_and_get_project] Using async_db_session: {async_db_session!r}")

 project_schema = ProjectCreate(
 name="Test Project Alpha", description="Alpha Test Description")

 logger.debug("[DEBUG test_create_and_get_project] Calling crud_projects.create_project...")
 db_project = await crud_projects.create_project(db=async_db_session, project=project_schema)
 logger.debug(f"[DEBUG test_create_and_get_project] crud_projects.create_project returned: {db_project!r}")
 ...
 logger.debug("[DEBUG test_create_and_get_project] Calling crud_projects.get_project...")
 retrieved_project = await crud_projects.get_project(
 db=async_db_session, project_id=db_project.id)
 logger.debug(f"[DEBUG test_create_and_get_project] crud_projects.get_project returned: {retrieved_project!r}")

 assert retrieved_project is not None
 assert retrieved_project.id == db_project.id
 assert retrieved_project.name == project_schema.name

 logger.debug("[DEBUG test_create_and_get_project] Calling crud_projects.get_project_by_name...")
 retrieved_by_name = await crud_projects.get_project_by_name(
 db=async_db_session, name=project_schema.name)
 logger.debug(f"[DEBUG test_create_and_get_project] crud_projects.get_project_by_name returned: {retrieved_by_name!r}")
...
async def test_delete_project_with_tasks_and_mock_print(async_db_session: AsyncSession):
 ...
 # Use a new session to ensure changes are reflected
 from .database import AsyncSessionLocal # Import the session maker
 async with AsyncSessionLocal() as new_session:
 tasks_after_delete = await crud_tasks.get_tasks(new_session, project_id=project_id)
