...
def setup_env():
    ...
    # Find the backend directory and its parent
    backend_dir = os.path.abspath(os.path.dirname(__file__))
    parent_dir = os.path.dirname(backend_dir)  # Parent directory of backend
    
    # We need to run tests from the parent directory so imports work correctly
    os.chdir(parent_dir)
...
def run_test(test_path):
    ...
    # Add 'backend/' prefix to the test path if not already present
    if not test_path.startswith("backend/"):
        test_path = os.path.join("backend", test_path)
